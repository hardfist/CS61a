...
def analyze_tweet_sentiment(tweet):
    ...
    lst=[get_word_sentiment(w) for w in tweet_words(tweet)]
    lst2=[ sentiment_value(w) for w in lst if has_sentiment(w)]
    if not lst2:
        return make_sentiment(None)
    else:
        return make_sentiment(1.0*sum(lst2)/len(lst2))
...
def find_centroid(polygon):
    ...
    for i in range(0,len(polygon)-1):
        ...
        xi_1=latitude(polygon[i+1])
        yi=longitude(polygon[i])
        yi_1=longitude(polygon[i+1])
        area+=1.0/2*(xi*yi_1-xi_1*yi)
    if abs(area) < eps:
        ...
        for i in range(0,len(polygon)-1):
            ...
            yi=longitude(polygon[i])
            yi_1=longitude(polygon[i+1])
            x+=(xi+xi_1)*(xi*yi_1-xi_1*yi)/(6*area)
            y+=(yi+yi_1)*(xi*yi_1-xi_1*yi)/(6*area)
        return (x,y,abs(area))
...
def group_tweets_by_state(tweets):
    ...
    tweets_by_state = dict()
    pos_state=dict()
    for key,item in us_states.items():
        pos_state[find_state_center(item)]=key
    for tweet in tweets:
        pos=tweet_location(tweet)
        mindist=None
        minPos=None
        for p in pos_state.keys():
            if mindist is None or geo_distance(p,pos) < mindist:
                mindist=geo_distance(p,pos)
                minPos=p
        if tweets_by_state.get(pos_state[minPos]) is None:
            tweets_by_state[pos_state[minPos]]=[tweet]
        else:
            tweets_by_state[pos_state[minPos]].append(tweet)
    ...
    for state,tweets in tweets_by_state.items():
        ...
        for tweet in tweets:
            ...
            if has_sentiment(sentiment):
                ...
                cnt+=1
        if total != 0.0:
            averaged_state_sentiments[state]=total/cnt
        else:
            pass
...
if __name__ == '__main__':
    import doctest
    doctest.run_docstring_examples(group_tweets_by_state,globals())
